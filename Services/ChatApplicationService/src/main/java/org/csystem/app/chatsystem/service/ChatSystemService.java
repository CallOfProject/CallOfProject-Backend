...
public class ChatSystemService
{
    ...
    public UserDTO saveUser(UserSaveDTO userSaveDTO)
    {
        ...
        // login infos are empty
        return m_userMapper.toUserDTO(user, new LoginInfosDTO(List.of()));
    }

    public LoginInfoDTO saveLoginInfoByNickName(String nickname)
    {
        var user = doForDataService(() -> m_chatSystemAppDataHelper.findUserByNickName(nickname),
                "ChatSystemService::saveLoginInfoByNickName");

        return saveLoginInfoByUser(user.get());
    }
    ...
    {
        return doForDataService(() -> m_chatSystemAppDataHelper.existsUserByNickname(username), "ChatSystemService::existsUserByNickname");
    }
    ...
    private LoginInfoDTO saveLoginInfoByUser(User user)
    {
        var loginInfo = new LoginInfo();
        loginInfo.user = user;
        loginInfo.localDateTime = LocalDateTime.now();

        var savedLoginInfo = doForDataService(() -> m_chatSystemAppDataHelper.saveLoginInfo(loginInfo),
                "ChatSystemService::saveLoginInfo");

        return m_loginInfoMapper.toLoginInfoDTO(savedLoginInfo);
    }
}